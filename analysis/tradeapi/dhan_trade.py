...
class ScripMaster:
    ...
    def _load_and_save(self, *, save_filtered: bool):
        ...
        try:
            with pd.read_csv(LOCAL_CSV, usecols=SCRIP_COLS, chunksize=50_000, low_memory=False) as reader:
                for chunk in reader:
                    mask = (chunk['SEM_EXM_EXCH_ID'].isin(FILTER_EXCH) & chunk['SEM_INSTRUMENT_NAME'].isin(FILTER_SEG))
        ...
        df['_EXPIRY_DATE_STR'] = df['SEM_EXPIRY_DATE'].astype(str).str.split().str[0]
        
        df.set_index(['SEM_EXM_EXCH_ID', 'SEM_INSTRUMENT_NAME', 'SEM_TRADING_SYMBOL'], inplace=True)
    ...
    def lookup(self, symbol: str, exch: str, seg: str, expiry_date: str = '') -> tuple[Optional[str], int]:
        self._ensure_loaded()
        if self._df is None: return None, 0
...
def _adjust_price(base: float, perc: float, signal: str) -> float:
    if signal == 'BUY': return math.ceil(base * (1 + perc / 100))
    return math.floor(base * (1 - perc / 100))

@dataclass
class PriceLevels:
    entry: float; limit: float; stop_loss: float; target: float; trail: float
...
def compute_price_levels(raw_entry: float, signal: str) -> PriceLevels:
    ...
    stop_loss = _adjust_price(entry, STOP_LOSS_PERC, 'SELL' if signal == 'BUY' else 'BUY')
...
def _signal_to_opt(signal: str) -> str: return 'CE' if signal == 'BUY' else 'PE'
...
def build_option_symbol(base: str, expiry_date, strike: int, opt_type: str, fmt: str) -> str:
    return f'{base}-{_format_expiry(expiry_date, fmt)}-{strike}-{opt_type}'

@dataclass
class Instrument:
    symb: str; exch: str; seg: str
    expiry_date: str = ''; signal: str = ''; quant: int = 1
    entry_val: float = 0.0; trade_amount: float = 0.0
...
class DhanTrader:
    ...
    def _resolve_nse(self, symb: str, signal: str, entry_val: float, quant: int) -> Optional[Instrument]:
        ...
        if symb in nse_index:
            if self.cfg.get('index_order_mode') != 'OPT': return None
            sym_data = nse_index[symb] or {}
            expiry   = sym_data.get('expiry_date', def_expiry)
            ...
            final_symb = build_option_symbol(sym_data.get('symbol', symb), expiry, strike, opt_type, '%b%Y')
            return Instrument(symb=final_symb, exch='NSE', seg='OPTIDX', expiry_date=expiry, signal='BUY', quant=sym_data.get('quantity', def_quant))

        ord_mode = self.cfg.get('stock_order_mode', '')
        if ord_mode == 'EQ':
            return Instrument(symb=symb, exch='NSE', seg='EQUITY', signal=signal, quant=quant, entry_val=entry_val, trade_amount=self.cfg.get('def_trade_amount', 10000))
        if ord_mode == 'OPT':
            final_symb = build_option_symbol(symb, def_expiry,  _round_strike(entry_val), _signal_to_opt(signal), '%b%Y')
            return Instrument(symb=final_symb, exch='NSE', seg='OPTSTK', expiry_date=def_expiry, signal='BUY', quant=def_quant)
        return None

    def _resolve_mcx(self, symb: str, signal: str, entry_val: float) -> Optional[Instrument]:
        mcx_data   = self.cfg.get('mcx_comm', {})
        def_expiry = self.cfg.get('def_expiry_date', '')
        if symb not in mcx_data or self.cfg.get('comm_order_mode') != 'OPT': return None
        ...
        sig_strk = 'call_strike' if signal == 'BUY' else 'put_strike'
        strike = sym_data.get(sig_strk, strike)
        final_symb = build_option_symbol(sym_data.get('symbol', symb), expiry, strike, _signal_to_opt(signal), '%d%b%Y')
        return Instrument(symb=final_symb, exch='MCX', seg='OPTFUT', expiry_date=expiry, signal='BUY', quant=sym_data.get('quantity', self.cfg.get('def_quantity', 1)))

    def resolve_instrument(self, symb: str, exch: str, signal: str, quant: int, entry_val: float) -> Optional[Instrument]:
        inst = self._resolve_nse(symb, signal, entry_val, quant) if exch == 'NSE' else self._resolve_mcx(symb, signal, entry_val) if exch == 'MCX' else None
        if inst is None:
    ...
    def place_super_order(self, symb: str, exchange_seg: str, lot_size: int, sec_id: str, signal: str, quant: int, entry_val: float, trade_amount: float):
        if entry_val == 0:
            payload = {'dhanClientId': CLIENT_ID, 'correlationId': f'auto_{CLIENT_ID}', 'transactionType': signal, 'exchangeSegment': exchange_seg, 'productType': 'INTRADAY', 'orderType': 'MARKET', 'validity': 'DAY', 'securityId': sec_id, 'quantity': quant * lot_size, 'price': 0}
            print(f'##### MARKET | {symb} | {sec_id} | {signal} | qty={quant * lot_size}')
        ...
        print(f'##### SUPER | {symb} | {sec_id} | {signal} | qty={total_quant} | entry={levels.entry} | amt={round(total_quant * levels.entry)}')
        
        payload = {
            'dhanClientId': CLIENT_ID, 'correlationId': f'auto_{CLIENT_ID}', 'transactionType': signal, 'exchangeSegment': exchange_seg, 
            'productType': 'INTRADAY', 'orderType': 'LIMIT', 'validity': 'DAY', 'securityId': sec_id, 'quantity': total_quant, 
            'price': levels.limit, 'stopLossPrice': levels.stop_loss, 'trailingJump': levels.trail
        }
    ...
    def fire_trade(self, symb: str, exch: str, signal: str, quant: int = 1, entry_val: float = 0):
        ...
        sec_id, lot_size = self.scrip.lookup(inst.symb, inst.exch, inst.seg, inst.expiry_date)

        if sec_id is None: return

        self.place_super_order(inst.symb, exchange_seg, lot_size, sec_id, inst.signal, inst.quant, inst.entry_val, inst.trade_amount)
