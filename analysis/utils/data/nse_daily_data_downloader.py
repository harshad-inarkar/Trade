...
class DownloaderConfig:
    ...
    api_url: str            = 'https://www.nseindia.com/api/live-analysis-most-active-underlying?csv=true'
...
class NSEDailyDownloader:
    ...
    def _calculate_intervals(self, tf=1, start_time_str=None, end_time_str=None) -> float:
    ...
    def _check_valid_session(self, curr_time: str) -> tuple[bool, str, bool]:
        ...
        if valid_flag and interval > max_intervals:
            print(f"Current timestamp {curr_time} passed end session time {self.config.end_session}")
    ...
    def _read_gcp_state(self) -> str:
        if self.gcp_state_file.exists():
            try:
                with open(self.gcp_state_file, "r") as f:
                    return f.read().strip()
