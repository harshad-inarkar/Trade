...
class CacheManager:
    ...
    def _load_csv_files(
        ...
        sv_rf = (
            self._seed_rma_fast[MIN_TF] if (incremental and from_index > 1) else None
        )
        sv_rs = (
            self._seed_rma_slow[MIN_TF] if (incremental and from_index > 1) else None
        )
        sv_rb = (
            self._seed_rma_base[MIN_TF] if (incremental and from_index > 1) else None
        )
        sv_prf = (
            self._seed_price_rma_fast[MIN_TF]
            if (incremental and from_index > 1)
            else None
        )
        sv_prs = (
            self._seed_price_rma_slow[MIN_TF]
            if (incremental and from_index > 1)
            else None
        )
        vol, rma_fast, rma_slow, rma_base, price_rma_fast, price_rma_slow = (
            post_process(
                ...
                seed_rma_slow=sv_rs,
                seed_rma_base=sv_rb,
                seed_price_rma_fast=sv_prf,
                seed_price_rma_slow=sv_prs,
                vltp_1indexed=vltp,
                seed_slot=slot,
            )
        )
        ...
        with ThreadPoolExecutor(max_workers=2) as ex:
            ...
            for tf_str, fut in futures.items():
                result[tf_str] = fut.result()  # re-raises any worker exception
    ...
    def _load_derived_tf(
        ...
        *,
        ...
        sv_rf = (
            self._seed_rma_fast.get(tf_str) if (incremental and from_idx > 1) else None
        )
        sv_rs = (
            self._seed_rma_slow.get(tf_str) if (incremental and from_idx > 1) else None
        )
        sv_rb = (
            self._seed_rma_base.get(tf_str) if (incremental and from_idx > 1) else None
        )
        sv_prf = (
            self._seed_price_rma_fast.get(tf_str)
            if (incremental and from_idx > 1)
            else None
        )
        sv_prs = (
            self._seed_price_rma_slow.get(tf_str)
            if (incremental and from_idx > 1)
            else None
        )
