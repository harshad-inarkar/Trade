...
CH_TSF, CH_TS, CH_VCUM, CH_VOL, CH_VSLOW, CH_VFAST, CH_VBASE, CH_PRICE = (
    0,
    1,
    2,
    3,
...
def discover_files(data_dir, last_n_days=None):
    ...
    if (
        last_n_days is not None
        and last_n_days > 0
        and len(sorted_dates_all) > last_n_days
    ):
...
def read_csv_files_to_arrays(
    ...
    file_col = {
        finfo["filename"]: fast_col_index(finfo["date_obj"]) for finfo in sorted_files
    }
    ...
    if known_symbols is not None:
        ...
        for i in range(len(sorted_files) - 1, 0, -1):
            if sorted_files[i]["date_obj"].strftime("%d%m%Y") != sorted_files[i - 1][
                "date_obj"
            ].strftime("%d%m%Y"):
            ...
            with open(finfo["filename"], encoding="utf-8-sig") as f:
                ...
                for row in reader:
                    ...
                    if si is None:
                        ...
                        sym_to_idx[sym] = si
                        vcum_arr = np.vstack(
                            [vcum_arr, np.full((1, vcum_arr.shape[1]), np.nan)],
                        )
                        vltp_arr = np.vstack(
                            [vltp_arr, np.full((1, vltp_arr.shape[1]), np.nan)],
                        )
                    try:
                        vcum_arr[si, n_file] = float(row[val_i])
                        vltp_arr[si, n_file] = float(row[ltp_i].strip('"'))
...
def post_process(
    ...
    if vltp_1indexed is not None:
        if seed_price_rma_fast is not None and from_index > 1:
            ...
            price_rma_fast = compute_price_rma(
                vltp_1indexed,
                from_index,
                total_files,
                rma_fast_len,
            ...
            price_rma_slow = compute_price_rma(
                vltp_1indexed,
                from_index,
                total_files,
                rma_slow_len,
...
def numpy_to_cache_rows(header, ts_list, tsf_list, num_matrix):
    ...
    for i in range(num_matrix.shape[0]):
        ...
        row[CH_VBASE] = base
        row[CH_VSLOW] = (
            0 if base == 0 else (float(num_matrix[i, VOL_SLOW]) * 100) / base
        )
        row[CH_VFAST] = (
            0 if base == 0 else (float(num_matrix[i, VOL_FAST]) * 100) / base
        )
